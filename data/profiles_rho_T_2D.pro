...
P=P-(Bx^2.0+By^2.0)/2.d0
P=(gamma-1.d0)*P

beta=dblarr(n1)

beta=((Bx*sqrt(mu)*1.0e4)^2.0+(By*sqrt(mu)*1.0e4)^2.0)/2.0/P
...
AXIS, 2.05d0,alog10(T(4)), YRANGE=[alog10(min(T)), alog10(max(T))], YAXIS=1, $
      /SAVE, charsize=1.5, ytitle=label_T, xtitle=label_Z
oplot, z(xb:xe)/scale, smooth(alog10(T(5:n_elements(T)/2-1)),40)
