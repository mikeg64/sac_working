...
for ij=0, n_elements(x0n)-1 do begin

     xc=interpol(dindgen(nxe-nxs+1),x,x0n[ij])
     yc=interpol(dindgen(nye-nys+1),y,y0n[ij])
     zc=interpol(dindgen(nz-nzs+1),z,z0n[ij])

     Vvx=interpolate(vx/rho, zc, xc, yc)
     Vvy=interpolate(vy/rho, zc, xc, yc)
     Vvz=interpolate(vz/rho, zc, xc, yc)
     
     x0=x0n[ij]+Vvx*dt/scale
     y0=y0n[ij]+Vvy*dt/scale
     z0=z0n[ij]+Vvz*dt/scale   
     print, Vvx, VVy,VVz, dt, z0n[ij], x0n[ij], y0n[ij]       
     x0n[ij]=x0
     y0n[ij]=y0
     z0n[ij]=z0
;if Rr^2.d0-((y0_s-mi_y-(ma_y-mi_y)/2.d0)^2.d0+(x0_s-mi_x-(ma_x-mi_x)/2.d0)^2.d0) ge aas then begin
while z0 ge mi_z do begin      
...
image_p = TVRD_24()
write_png,'/data/ap1vf/png/3D/tube/test_120s/test_move/'+indexss+'.png',image_p, red,green, blue
