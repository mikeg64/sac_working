...
print, 'tuta'
openr,1,'/data/ap1vf/2_6Mnzx1976400B41.out',/f77_unf
...
wset,0
!p.multi = [0,2,0,0,1]
tvframe,sabz_t*sqrt(mu)*1.0e4,/sample, /bar,title='Bx total [G]', xtitle='x [Mm]', ytitle='z [Mm]',charsize=1.0, $
...
tvframe,sabx_t*sqrt(mu)*1.0e4,/sample, /bar,title='Bx total [G]',xtitle='x [Mm]', ytitle='z [Mm]',charsize=1.0, $
        xrange=[min(xx)/scale, max(xx)/scale],yrange=[min(zz)/scale, max(zz)/scale] 
...
image_p = TVRD_24()
write_png,'/data/ap1vf/png/REAL/BztBxt_P30_R100_A500_B45_Dx4_Dz4_6_1976_400/'+indexss+'.png',image_p, red,green, blue
