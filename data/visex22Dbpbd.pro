...
Cosph=abx*abx0+abz*abz0 

AbsB=sqrt(abx0^2.d0+abz0^2.d0)

Bpl=Cosph/AbsB

Bpl(where(abs(abz0) le 1.0d-4))=0.d0

Cosph=abx*abz0 -abz*abx0 

Bpd=Cosph/AbsB

Bpd(where(abs(abz0) le 1.0d-4))=0.d0
...
wset,0
!p.multi = [0,2,0,0,1]
tvframe,Bpl*sqrt(mu)*10000.d0,/sample, /bar,title='!3b !I| |!N!3 [G]', xtitle='x [Mm]', ytitle='z [Mm]',charsize=1.0, $
...
tek_color
contour, beta*sqrt(mu)*10000.d0,xx/scale,zz/scale, LEVELS = [0.01,0.1,1.0, 10.0], $
         C_Annotation = ['100.0','10.0','1.0','0.1'], /overplot, /FOLLOW	  

;************************** V pd ***********************************
 
tvframe,Bpd,/sample, /bar,title='b!I!Mx!N!3 [G]',xtitle='x [Mm]', ytitle='z [Mm]',charsize=1.0, $
        xrange=[xmin/scale, xmax/scale],yrange=[zmin/scale, zmax/scale] 
...
contour, beta,xx/scale,zz/scale, LEVELS = [0.01,0.1,1.0, 10.0], $
 ...
 ss='time ='+strTrim(string(time)*.png
 ,1)
...
image_p = TVRD_24()
write_png,'/data/ap1vf/png/bpbpdB0_P30_R180_A500_B1050_Dx1_Dz1/'+indexss+'.png',image_p, red,green, blue
