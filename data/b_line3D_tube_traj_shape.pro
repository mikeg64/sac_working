...
WINDOW, /FREE, /PIXMAP, COLORS=256 & WDELETE, !D.WINDOW
   ...
   Window, 3, Title='Shape', xsize=900,ysize=300, $
           XPOS = 850, YPOS = 150

   Window, 4, Title='Vphi Vr', xsize=900,ysize=300, $
           XPOS = 250, YPOS = 150

   Window, 5, Title='Vphi test', xsize=700,ysize=300, $
           XPOS = 150, YPOS = 180
...
;traj_proj=dr+'/ends_moves_three_full_vect/traj_proj_shape/'
;vz_slice=dr+'/ends_moves_three_full_vect/vz_slice/'

traj=dr+'/test_phi/traj_three_shape/'
traj_proj=dr+'/test_phi/traj_proj_shape/'
vz_slice=dr+'/test_phi/vz_slice/'
...
;***************** begin  positions for SHAPE ***************************
if ijs eq 0 then begin
...
zcs_0=dblarr(1)
   ...
   xcs_0[0]=rr*cos(phi)+xx(n1-1,n2/2-1,n3-1,1)
   xcs_0[0]=xcs_0[0]/scale
   
   ycs_0[0]=rr*sin(phi)+xx(n1-1,n2-1,n3/2-1,2)
   ycs_0[0]=ycs_0[0]/scale
...
while phi le 2.d0*!Pi do begin
   ...
   xcs_0=[xcs_0,(rr*cos(phi)+xx(n1-1,n2/2-1,n3-1,1))/scale]
   ycs_0=[ycs_0,(rr*sin(phi)+xx(n1-1,n2-1,n3/2-1,2))/scale]
   zcs_0=[zcs_0,z(nnz)]
...
Vvx_s=dblarr(n_elements(xcs_0))
Vvy_s=Vvx_s
Vvz_s=Vvx_s

endif

 for ii=0, n_elements(xcs_0)-1 do begin 
     xcs_n=interpol(dindgen(nxe-nxs+1),x,xcs_0[ii])
     ycs_n=interpol(dindgen(nye-nys+1),y,ycs_0[ii])
     zcs_n=interpol(dindgen(nz-nzs+1),z,zcs_0[ii])

     Vvx_s[ii]=interpolate(vx/rho, zcs_n,xcs_n,ycs_n)
     Vvy_s[ii]=interpolate(vy/rho, zcs_n,xcs_n,ycs_n)
     Vvz_s[ii]=interpolate(vz/rho, zcs_n,xcs_n,ycs_n)
     
     xcs_0n[ii]=xcs_0[ii]+Vvx_s[ii]*dt/scale
     ycs_0n[ii]=ycs_0[ii]+Vvy_s[ii]*dt/scale
     zcs_0n[ii]=zcs_0[ii]+Vvz_s[ii]*dt/scale     

vaa=100.d0 ;6000.d0 
     xcs_0nn[ii]=xcs_0[ii]+Vvx_s[ii]*vaa*dt/scale
     ycs_0nn[ii]=ycs_0[ii]+Vvy_s[ii]*vaa*dt/scale
     zcs_0nn[ii]=zcs_0[ii]+Vvz_s[ii]*vaa*dt/scale     

endfor

;***************** end  positions for SHAPE ***************************


;****************** begin Vphi calculation ********************************************
Vr_s=dblarr(n_elements(Vvx_s))
Vphi_s=dblarr(n_elements(Vvx_s))
phi_s=dblarr(n_elements(Vvx_s))

for ii=0, n_elements(Vphi_s)-1 do begin
;***** radius ************

 
xxc=xx(n1-1,n2/2-1,n3-1,1)/scale-xcs_0[ii] ; x center
yyc=xx(n1-1,n2-1,n3/2-1,2)/scale-ycs_0[ii] ; y center

rr=sqrt(xxc^2+yyc^2)

 vr_s[ii]= Vvx_s[ii]*xxc/rr+Vvy_s[ii]*yyc/rr
 vphi_s[ii]=-Vvx_s[ii]*yyc/rr+Vvy_s[ii]*xxc/rr
 
 phi_s[ii]=acos(xxc/rr) 

endfor

if ijss eq 0 then begin
 T_Vphi_s=dblarr(1)
 T_Vphi_s[0]=total(Vphi_s)/n_elements(Vphi_s)
 ijss=1
 
 Vphi_test[0]=Vphi_s[5]
 
endif else begin
 T_Vphi_s=[T_Vphi_s, total(Vphi_s)/n_elements(Vphi_s)]
 
 Vphi_test=[Vphi_test, Vphi_s[5]]
endelse


;****************** end Vphi calculation **********************************************
...
max_Vphi=2500.d0 ;900.d0
min_Vphi=-2500.d0; -900.d0
;****************** begin plot shape **********************************************
for i=0 , n_elements(xcs_0)-1 do begin
 
 cl=color_max-(max_Vphi-vphi_s[i])/(max_Vphi-min_Vphi)*(color_max-color_min)

 PLOTS, xcs_0[i], ycs_0[i], zcs_0[i], color=cl, psym=8,  /T3D
 PLOTS, [xcs_0[i],xcs_0nn[i]], [ycs_0[i],ycs_0nn[i]], [zcs_0[i],zcs_0nn[i]], color=cl, /T3D
...
VELOVECT, reform(vx(12,*,*)/rho(12,*,*)), reform(vy(12,*,*)/rho(12,*,*)), charsize=2 ;
...
;--------------------
   ...
   css=2.0
   plot, xcs_0[0:n_elements(xcs_0)-1],ycs_0[0:n_elements(ycs_0)-1], title='x-y', /nodata, $
         charsize=css, XRANGE=[mi_x, ma_x], YRANGE=[mi_y, ma_y], $
	 xtitle='x', ytitle='y'
   ...
   for i=0 , n_elements(xcs_0)-1 do begin
     ...
     cl=color_max-(max_Vphi-vphi_s[i])/(max_Vphi-min_Vphi)*(color_max-color_min)
     plots, xcs_0[i],ycs_0[i], color=cl, psym=8
      
     PLOTS, [xcs_0[i],xcs_0nn[i]], [ycs_0[i],ycs_0nn[i]], color=cl
   
   endfor
   

   plot, xcs_0[0:n_elements(xcs_0)-1],zcs_0[0:n_elements(zcs_0)-1], title='x-z', /nodata, $
         charsize=css, XRANGE=[mi_x, ma_x], YRANGE=[0.3, 0.5], $
	 xtitle='x', ytitle='z'	 
   ...
   for i=0 , n_elements(xcs_0)-1 do begin
     ...
     plots, xcs_0[i],zcs_0[i], color=cl,psym=8
     PLOTS, [xcs_0[i],xcs_0nn[i]], [zcs_0[i],zcs_0nn[i]], color=cl
     
   endfor
   

   plot, ycs_0[0:n_elements(ycs_0)-1],zcs_0[0:n_elements(zcs_0)-1], title='y-z', /nodata, $
         charsize=css, XRANGE=[mi_y, ma_y], YRANGE=[0.3, 0.5], $
	 xtitle='y', ytitle='z'	 
   ...
   for i=0 , n_elements(xcs_0)-1 do begin
     ...
     plots, ycs_0[i],zcs_0[i], color=cl, psym=8
     PLOTS, [ycs_0[i],ycs_0nn[i]], [zcs_0[i],zcs_0nn[i]], cl
...
plot, phi_s, Vphi_s, xtitle='phi', ytitle='Vphi', color=200, charsize=css
oplot, phi_s, Vphi_s, psym=8
plot, phi_s, Vr_s,  xtitle='phi', ytitle='Vr', color=200, charsize=css
oplot, phi_s, Vr_s, psym=8
plot, T_Vphi_s,  xtitle='n', ytitle='T_Vphi_s', color=200, charsize=css
oplot, T_Vphi_s, psym=8
...
WSet, 5
!P.multi=0
plot, Vphi_test,  ytitle='Vphi[5]', color=200, charsize=1.2
...
save, filename='T_Vphi_s.saw', T_Vphi_s, Vr, phi_s, Vphi_test
