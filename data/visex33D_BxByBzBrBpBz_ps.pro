...
tvframe,br(hxmin:hxmax,hymin:hymax)*sqrt(mu)*1.0e4,/bar,/sample, $
        xtitle='x [Mm]', ytitle='y [Mm]', charsize=cs, $
	xrange=[xx(hxmin)/scale, xx(hxmax)/scale], $
	yrange=[yy(hymin)/scale, yy(hymax)/scale],$	
	BTITLE='b!D r!3!N [Gauss]'

tvframe,vr(hxmin:hxmax,hymin:hymax),/bar,/sample, $
        xtitle='x [Mm]', ytitle='y [Mm]', charsize=cs, $
	xrange=[xx(hxmin)/scale, xx(hxmax)/scale], $
	yrange=[yy(hymin)/scale, yy(hymax)/scale], $
	BTITLE='V!D r!3!N [m/s]'	

tvframe,bphi(hxmin:hxmax,hymin:hymax)*sqrt(mu)*1.0e4,/bar,/sample, $
        xtitle='x [Mm]', ytitle='y [Mm]', charsize=cs, $
	xrange=[xx(hxmin)/scale, xx(hxmax)/scale], $
	yrange=[yy(hymin)/scale, yy(hymax)/scale], $
	BTITLE='b!D!7 u!3!N [Gauss]'	
	
tvframe,vphi(hxmin:hxmax,hymin:hymax),/bar,/sample, $
        xtitle='x [Mm]', ytitle='y [Mm]', charsize=cs, $
	xrange=[xx(hxmin)/scale, xx(hxmax)/scale], $ 
	yrange=[yy(hymin)/scale, yy(hymax)/scale], $
...
ss='time ='+strTrim(string(FORMAT='(6F10.2)', time),2)
 xyouts,20,5, ss, /device, color=0, charsize=cs
